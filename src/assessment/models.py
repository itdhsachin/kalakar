...
class AssessmentUpload(models.Model):
    ...
    def __str__(self):
        ...
        return (
            f"Submission {self.id} by {self.user_id.username}"
            f"(Teacher: {self.assigned_teacher.username if self.assigned_teacher else 'Not Assigned'})"  # pylint: disable=inconsistent-quotes
